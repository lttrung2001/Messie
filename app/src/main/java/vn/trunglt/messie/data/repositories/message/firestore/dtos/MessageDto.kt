...
data class MessageDto(val sender: String, val text: String, val timestamp: Long) {
    fun toMessageModel(): MessageModel {
        return MessageModel(
            id = "", // Firestore không trả về ID trong snapshot listener, bạn có thể xử lý nó ở nơi khác nếu cần
            sender = sender,
            text = text,
            timestamp = timestamp
        )
